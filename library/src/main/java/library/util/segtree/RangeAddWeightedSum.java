...
/**
 * 重み付き区間加算・重み付き区間和取得を行うデータ構造。
 * 整数列 A に対し、Σ A_i c_i を計算量 O(log N) で処理する。
 * (c_i は事前に与えられた重み)
 */
public class RangeAddWeightedSum {
	 ...
	 * 計算量: O(N)
	 */
	public RangeAddWeightedSum(long[] weights) {
		...
		this.seg = new IndexedLazySegTreelonglong(N, new IndexedLazySegTreeStrategy_longlong() {
			...
			public long mergeAX(long a, long x, int l, int r) {
				return x + a * (C[Math.min(r, N)] - C[Math.min(l, N)]);
			}
		});
	}
	...
	public void build(long[] A) {
		...
		for (int i = 0; i < N; i++) weighted[i] = A[i] * weights[i];
		seg.build(weighted);
	}
	...
	// 未テスト
	public void dump() {
		System.out.println("RangeAddWeightedSum { N: " + N + ", weights: " + java.util.Arrays.toString(weights) + " }");
