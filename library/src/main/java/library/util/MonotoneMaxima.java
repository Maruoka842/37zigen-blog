...
public class MonotoneMaxima {
	...
	public static int[] rowMaxima(int H, int W, CostFunction f) {
		int[]ans=new int[H];
		Arrays.fill(ans, -1);
		solve(0, H, 0, W, ans, f);
		return ans;
	...
	private static void solve(int x0, int x1, int y0, int y1, int[]ans, CostFunction f) {
		...
		for (int i = y0; i < y1; i++) {
			...
			if (maxArg==-1 || maxValue < value) {
				maxValue = value;
				maxArg = i;
			}
		}
		ans[mid]=maxArg;
		solve(x0, mid, y0, maxArg+1, ans, f);
		solve(mid+1, x1, maxArg, y1, ans, f);
	}
