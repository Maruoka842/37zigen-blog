...
/**
 * 8近傍パス輪郭線DP (8-Neighbor Path Frontier DP) におけるパス・サイクルの状態を管理するクラス。
 ...
 * @author Jules
 */
public final class PathFrontier8 {
    ...
    public PathFrontier8(byte[] parent, int c, boolean hasDead) {
        this(parent, c, hasDead, 0, 0);
    }

    // 未テスト
    public PathFrontier8(byte[] parent, int c, boolean hasDead, int deadEnds, int cycles) {
        this.parent = parent;
        this.c = c;
    ...
    // 未テスト
    public static PathFrontier8 getInitialState(int M) {
        return new PathFrontier8(new byte[M + 1], 0, false, 0, 0);
    }

    // 未テスト
    public PathFrontier8 chminDeadEnds(int val) {
        if (this.deadEnds <= val) return this;
        return new PathFrontier8(parent, c, hasDead, val, cycles);
    }

    // 未テスト
    public PathFrontier8 chminCycles(int val) {
        if (this.cycles <= val) return this;
        return new PathFrontier8(parent, c, hasDead, deadEnds, val);
    }
     ...
     * 生成された遷移後の有効な PathFrontier8 の集合を返します。
     *
     * @param dirs 試行する接続方向の集合
     * @return 遷移後の有効な PathFrontier8 の反復可能（Iterable）オブジェクト
     */
    // 未テスト
    public Iterable<PathFrontier8> getPossibleConnections(Direction... dirs) {
     ...
     */
    // 未テスト
    public Iterable<PathFrontier8> getPossibleConnections(byte startId, Direction... dirs) {
        java.util.List<PathFrontier8> list = new java.util.ArrayList<>();
        int n = dirs.length;
        for (int mask = 0; mask < (1 << n); mask++) {
            ...
            if (ok) {
                PathFrontier8 next = builder.build(startId);
                if (next != null) {
                    list.add(next);
                }
            }
    ...
    // 未テスト
    public PathFrontier8 nextWithoutVertex() {
        return nextWithoutVertex((byte) 0);
    }

    // 未テスト
    public PathFrontier8 nextWithoutVertex(byte startId) {
    ...
    // 未テスト
    public PathFrontier8 shift() {
        return shift((byte) 0);
    }

    // 未テスト
    public PathFrontier8 shift(byte startId) {
        ...
        shifted[0] = 0;
        return new PathFrontier8(shifted, 0, hasDead, nextDeadEnds, cycles);
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof PathFrontier8)) return false;
        PathFrontier8 that = (PathFrontier8) o;
    ...
    public static final class Builder {
        ...
        public Builder(PathFrontier8 frontier) {
        ...
        // 未テスト
        public PathFrontier8 build() {
            return build((byte) 0);
        }

        // 未テスト
        public PathFrontier8 build(byte startId) {
            int nextDeadEnds = deadEnds;
            ...
            return new PathFrontier8(canonicalParent, c + 1, hasDead, nextDeadEnds, cycles + addedCycles);
        }
    }
}
