...
public class LongVertexValueUnionFind {
	int[] parent;
	long[] vertexValues;
	...
	public LongVertexValueUnionFind(int n, LongBinaryOperator op) {
		parent=new int[n];
		vertexValues = new long[n];
	...
	public void union(int left, int right) {
		...
		if (-parent[right]>-parent[left]) {
			...
			parent[left]=right;
			vertexValues[right] = op.applyAsLong(vertexValues[left], vertexValues[right]);
		} else {
			parent[left]+=parent[right];
			parent[right]=left;
			vertexValues[left] = op.applyAsLong(vertexValues[left], vertexValues[right]);
		}
	...
	public void mulRight(int v, int t) {
		vertexValues[root(v)] = op.applyAsLong(getVertexValue(root(v)), t);
	}
	 ...
	 * @param a
	 */
	public void set(int v, long a) {
		v=root(v);
		vertexValues[root(v)] = a;
	}
	
	public long getVertexValue(int x) {
	...
	/**
	 * LongVertexValueUnionFindの現在の状態を、連結成分ごとに文字列として表す。
	 * <ul>
	 *   <li>事前条件: 特になし。</li>
	...
	/**
	 * LongVertexValueUnionFindの現在の状態を、連結成分ごとに標準出力へ出力する。
	 * <ul>
	 *   <li>事前条件: 特になし。</li>
